...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['city_id']) && !empty($_FILES['city_image']['name'])) {
        ...
        $sql = "SELECT city_slug, city_name FROM non_coastal_cities WHERE city_id = :city_id";
        ...
        if ($city) {
            ...
            if (move_uploaded_file($_FILES["city_image"]["tmp_name"], $imagePath)) {
                $sql = "INSERT INTO city_images_noCoastal (city_id, city_slug, image_path) VALUES (:city_id, :city_slug, :image_path)";
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':city_id', $cityId);
        ...
        echo "⚠️ ID de la ville ou image non spécifié.";
    }
}
...
if (isset($_GET['delete_image_id'])) {
    ...
    $sql = "SELECT image_path FROM city_images_noCoastal WHERE image_id = :image_id";
    $stmt = $pdo->prepare($sql);
    ...
    if ($image) {
        ...
        $sqlDelete = "DELETE FROM city_images_noCoastal WHERE image_id = :image_id";
...
if (!empty($_GET['city_id'])) {
    ...
    $sql = "SELECT * FROM city_images_noCoastal WHERE city_id = :city_id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':city_id', $cityId);
    $stmt->execute();
...
<?php
$sql = "SELECT * FROM city_images_noCoastal ORDER BY image_id DESC";
$stmt = $pdo->prepare($sql);
$stmt->execute();
        ...
        <tbody>
            <?php foreach ($recentImages as $index => $image): ?>
                <tr>
                    ...
                    <td>
                        <?php
                        $sqlCity = "SELECT city_name FROM non_coastal_cities WHERE city_id = :city_id";
...
<?php else: ?>
    <p>Aucune image récemment ajoutée.</p>
<?php endif; ?>
